...
MESH="../mesh/power_based_refined_mesh.msh"
MESH_META="../mesh/power_based_refined_mesh_metadata.json"
POWERMAP="../../powermap/powermap_v3/powermap_example.csv"
OUTPUT_DIR="result"

for f in "$OPT_JSON" "$MESH" "$MESH_META" "$POWERMAP"; do
    [[ -f "$f" ]] || { echo "[optimization] ERROR: missing input '$f' (run ../generate_power_based_mesh.sh first)" >&2; exit 1; }
done
mkdir -p "$OUTPUT_DIR"
